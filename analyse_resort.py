...
def analyse_solution(model, article_length, hull_params, article_priority):
    ...
    for v in model.getVars():
        ...
        # Resort-Wahl merken: w_<page>_<resort>
        if name.startswith("w_"):
            parts = name.split("_")
            if len(parts) >= 3 and abs(val - 1.0) <= tol:
                page = int(parts[1])
                resort = int(parts[2])
                chosen_resort[page] = resort
    ...
    _h("RESORT-SEQUENZ")
    for page in sorted(chosen_resort.keys()):
        print(f"  Seite {page:>3}: Resort {chosen_resort[page]}")
    ...
    for page in pages_to_print:
        ...
        resort = chosen_resort.get(page, None)
        resort_txt = f" | Resort: {resort}" if resort is not None else " | Resort: ?"
        print(f"\nSeite {page}{lay_txt}{resort_txt}{score_txt}")
    ...
    print("\nchosen_resort:")
    pprint(dict(sorted(chosen_resort.items())))
